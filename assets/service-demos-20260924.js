...
(function () {
  ...
  (function setupCompare() {
    ...
    function paint(pos) {
      ...
      var t = 1 - pos / 100;
      if (metrics.load) metrics.load.textContent = mix(before.load, after.load, t).toFixed(1).replace(".", ",") + " s";
      if (metrics.perf) metrics.perf.textContent = String(Math.round(mix(before.perf, after.perf, t)));
      if (metrics.err) metrics.err.textContent = String(Math.round(mix(before.err, after.err, t)));
